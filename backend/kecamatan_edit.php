...
<html>
   ...
   /** menerima data yang akan diubah */
    ...
    $edit = mysqli_query($conn, "SELECT * FROM kecamatan WHERE kecamatan_ID = '$kodekecamatan'");
    ...
    if (isset($_POST['ubah']))
    {
        ...
        if($namafoto == ""){
        mysqli_query($conn, "UPDATE kecamatan set kecamatan_NAMA='$kecamatanNAMA', kabupaten_ID='$kabupatenID' 
        WHERE kecamatan_ID='$kodekecamatan'"); 
        } else
        {
        mysqli_query($conn, "UPDATE kecamatan set kecamatan_NAMA='$kecamatanNAMA', kabupaten_ID='$kabupatenID', 
        kecamatan_FOTO='$namafoto' WHERE kecamatan_ID='$kodekecamatan'");
        }
        header("location: inputkecamatan.php");
    }
    /** $query = mysqli_query ($conn, "select * from kecamatan,kabupaten */
    /** where kecamatan.kabupaten_ID = kabupaten.kabupaten_ID"); */
    ...
    <body>
        ...
        <div class="col-10">
        <div class="judul">    
        <h2 style="padding-left:40px;">Input Kecamatan<h2>
        </div>
        <div class="judul2">    
        <small style="padding-left:40px;">Kecamatan</small>
        </div>
        <!-- membuat form input data kategori -->
         ...
         <div class="row mb-3 mt-5">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control" id="kecamatanID" name="inputID" value="<?php echo $row_edit["kecamatan_ID"]?>"readonly>
            </div>
         </div>
         <div class="row mb-3 mt-5">
         ...
         <div class="row mb-3 mt-5">
            ...
            <div class="col-sm-10">
                <select class="form-control" id="kabupatenID" name="kabupatenID">
                    <option><?php echo $row_edit["kabupaten_ID"]?></option>
         ...
         <div class="judul3">
            <h2 style="padding-left:40px;">Output Kecamatan<h2>
         </div>

         <!-- form pencarian data !-->
         <form method="POST">
            <div class="form-group row mt-5">
                <label for="search" class="col-sm-2">Cari Judul Kecamatan</label>
                <div class="col-sm-6">
                    <input type="text" name="search" class="form-control" id="search"
                    value="<?php if(isset($_POST["search"]))
                    {echo $_POST["search"];}?>" placeholder="Cari Judul Kecamatan">
                </div>
         ...
         <table class="table table-striped table-success table-hover mt-5">
            ...
            if(isset($_POST["kirim"]))
            {
              $search = $_POST["search"];
              $query = mysqli_query ($conn, "select * from kecamatan, kabupaten
              where kecamatan.kabupaten_ID = kabupaten.kabupaten_ID and 
              kecamatan_NAMA like '%".$search."%'");
            }else
            {
              $query = mysqli_query ($conn, "select * from kecamatan, kabupaten
              where kecamatan.kabupaten_ID = kabupaten.kabupaten_ID");
            ...
            <tr class="danger">
                ...
                <td>
                    <?php if($row['kecamatan_FOTO']==""){ echo "<img src='images/noimage.png'
                    width='88'/>";}else{?>
                    <img src="../admin/images/<?php echo $row['kecamatan_FOTO'] ?>" width="88" class="
                    img-responsive" />
                <?php }?>
                </td>
                <td>
                    <a href="kecamatan_edit.php?ubahkecamatan=<?php echo $row["kecamatan_ID"]?>" class="btn btn-success btn-sm" title="EDIT">
                ...
                <td>
                   <a href="kecamatan_hapus.php?hapuskecamatan=<?php echo $row["kecamatan_ID"]?>" class="btn btn-danger btn-sm" title="HAPUS">
